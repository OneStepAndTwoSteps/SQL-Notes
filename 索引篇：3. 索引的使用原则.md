...
### 3、在 WHERE 子句中，如果在 OR 前的条件列进行了索引，而在 OR 后的条件列没有进行索引，那么索引会失效。 如：
...
### 4、当我们使用 LIKE 进行模糊查询的时候,匹配的字段前不能加 %，如：
...
*   __判断索引列是否为 NOT NULL，往往需要走全表扫描，__ 因此我们最好在设计数据表的时候就将字段设置为 NOT NULL 约束,比如你可以将 INT 类型的字段，默认值设置为 0。将字符类型的默认值设置为空字符串 ('')。
